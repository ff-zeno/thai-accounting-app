...
| DB queries | `src/lib/db/queries/` (documents, transactions, payments, vendors, wht-certificates, vat-operations-ledger, reconciliation, reconciliation-metrics, reconciliation-rules, wht-filings, wht-rates, dashboard, ai-suggestions, ai-settings, vendor-aliases, bank-accounts, organizations, document-files, processor-settlements) |
...
| Inngest pipeline | `src/lib/inngest/functions/process-document.ts` (7-step extraction), `src/lib/inngest/functions/reconcile-document.ts` (7-layer match cascade), `suggest-rules.ts` (auto-rule suggestion), `ai-reconciliation-dispatcher.ts` (hourly cron), `ai-reconciliation-batch.ts` (per-org AI matching), `match-imported-transactions.ts` (transaction-first, settlements claim first), `match-settlements.ts` (payout matching) |
| Bank parsers | `src/lib/parsers/` (KBank CSV/PDF, generic CSV, balance validation) |
| Merchant settlements | `src/lib/parsers/settlement-csv.ts` (generic column mapper + the `gross − fee − feeVat = net` invariant), `src/lib/db/queries/processor-settlements.ts` (import, register, match writers), `src/app/(app)/income/settlements/` (register + import UI) |
| Payout matching | `src/lib/reconciliation/settlement-matcher.ts` (4-layer cascade: reference, exact, processor, batched), `src/lib/inngest/functions/match-settlements.ts`, `src/app/(app)/reconciliation/payouts/` (review queue). A settlement explains a bank deposit; net payout **never** feeds an output-VAT path — output VAT is owed on gross |
...
| App shell / nav | `src/lib/nav/structure.ts` (6 top-level entries — Home, Bank, Income, Expenses, Tax, Vendors — plus Settings in the footer; nav pins and the POS/Sales gate removed), `src/lib/nav/badges.ts` (nav badge counts + tax deadline chip, unit-tested), `src/lib/routes/documents.ts` (direction → section route, the one place the expense/income split is decided), `src/components/layout/` (app-sidebar, top-bar, mobile-tab-bar, route-tabs, sub-nav, org-switcher, locale-switcher, no-org-gate) |
