...
|---|---|---|
| `scope-reduction.md` | **Executed in the working tree 2026-08-03, uncommitted.** Strips the app to the weekly money loop after the owner reported the app was unusable through excess. Everything removed is registered in `docs/deferred-features.md`. The nav it left (Home, Documents, Bank, Tax, Vendors) was restructured on 2026-08-05 by the money-flow work below. | Golden-path e2e run and green; `drizzle/0003_premium_doctor_doom.sql` applied with owner authorization; committed. |
| Money-flow IA + merchant settlements (plan at `~/.claude/plans/toasty-leaping-squid.md`) | **Stages 1–5 executed 2026-08-05/06; stage 1 committed, stages 2–5 still in the working tree.** Nav restructured to Home · Bank · Income · Expenses · Tax · Vendors with redirect stubs; `processor_settlements` restored; settlement CSV ingest at `/income/settlements`; payout matching at `/reconciliation/payouts`. | `pnpm test:db` green (needs Docker Postgres); golden-path e2e green; the manual settlement-import-to-deposit walkthrough done; committed. |
